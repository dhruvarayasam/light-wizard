...
int main() {
    ...
    // make plane
    Plane* ptr = new Plane {10, 0x808080};
    shared_ptr<Geometry> plane_ptr (ptr);

    // collect scene geometry
    vector<shared_ptr<Geometry>> scene_geom;
    scene_geom.push_back(first);
    scene_geom.push_back(plane_ptr);

    // assimilate scene
    shared_ptr<Scene> scene_ptr = std::make_shared<Scene>(0x87CEEB, scene_geom, light, *ptr);
